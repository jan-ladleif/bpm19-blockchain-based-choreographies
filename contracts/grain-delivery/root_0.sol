...
contract Factory_GrainDelivery_Deposit is ChoreographyFactory {
    function create(bytes data)
        external
        returns (address newContract)
    {
        newContract = new GrainDelivery_Deposit(msg.sender, data);
        ChoreographyCreated(newContract, data);
    }
}

contract GrainDelivery_Deposit is BaseChoreography {
    // -------------------------------------------------------------------------
    // data object storage
    // -------------------------------------------------------------------------
    ...
    constructor(address addr, bytes memory data) public {
        ...
        assembly {sstore(depositAmount_amount_slot,or(and(sload(depositAmount_amount_slot),not(mul(0xffff, exp(256, depositAmount_amount_offset)))),mul(and(mload(add(data,2)),0xffff),exp(256, depositAmount_amount_offset))))}
        emit Created(data);
    }
        ...
        external
    {
        ...
        require (participants.hasRole(msg.sender, [1, 2, 2][task]));

        // check if task is enabled and consume token
        uint8 incoming = (uint8(1) << [0, 2, 2][task]);
        ...
        private
    {
        ...
        while (toVisit > 0) {
            ...
            for (uint8 i = 0; i < FLOW_COUNT; i++) {
                if (toVisit & ~uint8(0x5) & (uint8(1) << i) > 0) {
